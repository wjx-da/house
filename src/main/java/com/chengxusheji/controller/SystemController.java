...
public class SystemController { 
	...
	@ResponseBody
	public ServerResponse getCheckedCode(String custTelno, Integer type, HttpServletRequest request, HttpServletResponse response) {
		ServerResponse serverResponse = null;
		ServerResponse result ;

			serverResponse = customerService.getByEmail(custTelno);
			if(serverResponse.getStatus() == 1){
				result = com.chengxusheji.utils.ServerResponse.getFailed("用户已存在，请前往登录页面");
			}else {
				String checkCode = String.valueOf(new Random().nextInt(899999) + 100000);
				String message = "您的注册验证码为：" + checkCode;
				try {
					mailService.sendSimpleMail(custTelno, "注册验证码", message);
				} catch (Exception e) {
				}
